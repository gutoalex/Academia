...
function gerarDadosDemo() { if(!confirm("Gerar demo?")) return; const h=[]; const l=['A','B','C','D']; for(let i=0; i<10; i++){ const d=new Date(); d.setDate(d.getDate()-(9-i)); const k=20+(i*5); h.unshift({data:`${d.getDate()}/${d.getMonth()+1}/${d.getFullYear()}`, nome:`TREINO ${l[i%4]}`, timestamp:d.getTime(), exercicios:[{nome:"Supino Demo", sets:[{s:1,k:k,r:10}]}]}); } localStorage.setItem('workout_history', JSON.stringify(h)); alert("Feito!"); location.reload(); }
function abrirDetalhesData(t) { document.getElementById('history-date-title').innerText = t.data; const ul = document.getElementById('history-details-list'); ul.innerHTML = ''; document.getElementById('history-notes-display').innerText = t.notes ? `📝 "${t.notes}"` : ''; if(t.exercicios) { t.exercicios.forEach(ex => { const best = ex.sets.reduce((p, c) => (parseFloat(p.k)>parseFloat(c.k))?p:c); ul.innerHTML += `<li class="detail-item"><span>${ex.nome}</span><strong>${best.k}kg x ${best.r}</strong></li>`; }); } document.getElementById('history-modal').classList.remove('hidden'); }
...
function renderizarCalendario() { const c = document.getElementById('calendar-days'); const l = document.getElementById('calendar-month-year'); const h = JSON.parse(localStorage.getItem('workout_history')) || []; const ms = ["JAN","FEV","MAR","ABR","MAI","JUN","JUL","AGO","SET","OUT","NOV","DEZ"]; c.innerHTML = ''; if(modoVisualizacao==='semanal') { const d=dataReferencia.getDay(); const dom=new Date(dataReferencia); dom.setDate(dataReferencia.getDate()-d); const sab=new Date(dom); sab.setDate(dom.getDate()+6); l.innerText=`${dom.getDate()} ${ms[dom.getMonth()]} - ${sab.getDate()} ${ms[sab.getMonth()]}`; for(let i=0;i<7;i++){ const dt=new Date(dom); dt.setDate(dom.getDate()+i); criarDia(dt,h,c); } } else { l.innerText=`${ms[dataReferencia.getMonth()]} ${dataReferencia.getFullYear()}`; const ano=dataReferencia.getFullYear(); const mes=dataReferencia.getMonth(); const f=new Date(ano,mes,1).getDay(); const t=new Date(ano,mes+1,0).getDate(); for(let i=0;i<f;i++) c.appendChild(document.createElement('div')); for(let i=1;i<=t;i++) criarDia(new Date(ano,mes,i),h,c); } }
function criarDia(d,h,c) { const div=document.createElement('div'); div.className='calendar-day'; div.innerText=d.getDate(); const k=`${String(d.getDate()).padStart(2,'0')}/${String(d.getMonth()+1).padStart(2,'0')}/${d.getFullYear()}`; if(d.toDateString()===new Date().toDateString()) div.classList.add('today'); const t=h.find(x=>x.data===k); if(t){ const l=t.nome.split(' ').pop().toLowerCase(); div.classList.add(`workout-${l}`); div.onclick=()=>abrirDetalhesData(t); } c.appendChild(div); }
...
function renderizarGraficos() { const hist = JSON.parse(localStorage.getItem('workout_history')) || []; if(hist.length===0) return; const cnt={A:0,B:0,C:0,D:0}; hist.forEach(h=>{const l=h.nome.split(' ').pop(); if(cnt[l]!==undefined)cnt[l]++;}); new Chart(document.getElementById('chart-pie'), { type:'doughnut', data:{labels:['A','B','C','D'], datasets:[{data:[cnt.A,cnt.B,cnt.C,cnt.D], backgroundColor:['#00f0ff','#bd00ff','#00ff9d','#ff9100'], borderWidth:0}]}, options:{plugins:{legend:{position:'bottom', labels:{color:'#fff'}}}} }); const ult=hist.slice(0,15).reverse(); const lbs=ult.map(h=>h.data.substring(0,5)); const dA=[], dB=[], dC=[], dD=[]; ult.forEach(h=>{ let v=0; if(h.exercicios) h.exercicios.forEach(ex=>ex.sets.forEach(s=>v+=(parseFloat(s.k)||0)*(parseFloat(s.r)||0))); const l=h.nome.split(' ').pop(); dA.push(l==='A'?v:null); dB.push(l==='B'?v:null); dC.push(l==='C'?v:null); dD.push(l==='D'?v:null); }); new Chart(document.getElementById('chart-line'), { type:'line', data:{labels:lbs, datasets:[{label:'A',data:dA,borderColor:'#00f0ff',spanGaps:true},{label:'B',data:dB,borderColor:'#bd00ff',spanGaps:true},{label:'C',data:dC,borderColor:'#00ff9d',spanGaps:true},{label:'D',data:dD,borderColor:'#ff9100',spanGaps:true}]}, options:{scales:{y:{ticks:{color:'#666'},grid:{color:'#222'}},x:{ticks:{color:'#666'},grid:{display:false}}},plugins:{legend:{labels:{color:'#fff'}}}} }); }
